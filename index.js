...
async function installModules(modules) {
	...
	for (const m of modules) {
		try {
			...
			errConsole(`Error file ${colors.fg.red}${m.name}${colors.reset}: ${e}`);
...
async function goModules() {
	...
	if(toDownload.length > 0){
		...
		okConsole(`${colors.fg.green}${modules.length}${colors.reset}/${modules.length} modules already installed.`);
}
...
async function installFiles(files, contents) {
	const errorFiles = [];
	for (const f of files) {
		try {
			...
			const content = await github(`${contents}${f}`);
			await fs.promises.writeFile(f, content, 'utf8');
		} catch (e) {
			errConsole(`Error file ${colors.fg.red}${f}${colors.reset}: ${e}`);
			errorFiles.push(f);
		}
	}
	return errorFiles;
